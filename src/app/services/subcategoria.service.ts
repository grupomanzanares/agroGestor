...
export class SubcategoriaService {
  ...
  async obtenerLocal(tabla: string): Promise<Subcategoria[]> {
    ...
    try {
      ...
      if (result.values) {
        ...
        throw new Error("No se encontraron datos de las Subcategoriaes en la base de datos")
      }
    } catch (error) {
  ...
  comparacion(endPoint: string, tabla: string): Observable<{ update: Subcategoria[], create: Subcategoria[] }> {
    return forkJoin({
  ...
  async update(datosDiferentes: Subcategoria[], tabla: string) {
    ...
    try {
      for (const datos of datosDiferentes) {
        ...
        if (cambios.length > 0) {
          console.log(`Subcategoria con id ${datos.id} actualizado con exito, ${cambios.join(', ')}`)
        } else {
          console.log(`Subcategoria con id ${datos.id} no requiere de actualizacion`)
        }
      }
    } catch (error) {
      console.error('Error al actualizar las Subcategoriaes: ', error)
    }
  }

  async create(datosParaCrear: Subcategoria[], tabla: string) {
    const db = await this.sqlManagerService.getDbName();
