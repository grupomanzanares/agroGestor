...
import { forkJoin, from, last, lastValueFrom, map, Observable } from 'rxjs';
...
export class PrioridadService {
  ...
  obtenerVps(endPoint: string): Observable<Prioridad[]> {
    const token = localStorage.getItem('token')
    const headers = new HttpHeaders({
      Authorization: `Bearer ${token}`
    })
    return this.http.get<Prioridad[]>(`${this.apiUrl}${endPoint}`, { headers })
  }

  async obtenerLocal(tabla: string): Promise<Prioridad[]> {
    ...
    try {
      ...
      if (result.values) {
        ...
        throw new Error("No se encontraron datos de las prioridades en la base de datos")
      }
    } catch (error) {
  ...
  comparacion(endPoint: string, tabla: string): Observable<{ update: Prioridad[], create: Prioridad[] }> {
    return forkJoin({
  ...
  async update(datosDiferentes: Prioridad[], tabla: string) {
    ...
    try {
      for (const datos of datosDiferentes) {
        ...
        if (cambios.length > 0) {
          console.log(`Prioridad con id ${datos.id} actualizado con exito, ${cambios.join(', ')}`)
        } else {
          console.log(`Prioridad con id ${datos.id} no requiere de actualizacion`)          
        }
      }
    } catch (error) {
  ...
  async create(datosParaCrear: Prioridad[], tabla: string){
    const db = await this.sqlService.getDbName()
  ...
  async sincronizar(endPoint: string, tabla: string) {
    try {
      ...
      if (update.length > 0) {
        ...
        console.log('Prioridad actualizada con exito')
      } else {
      ...
      if(create.length > 0 ) {
        ...
        console.log('Datos de prioridad insertados correctamente')
      }
