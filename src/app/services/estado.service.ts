...
export class EstadoService {
  ...
  obtenerVps(endPoint: string): Observable<Estado[]>{
    const token = localStorage.getItem('token')
    const headers = new HttpHeaders({
      Authorization: `Bearer ${token}`
    })
    return this.http.get<Estado[]>(`${this.apiUrl}${endPoint}`, { headers })
  }

  async obtenerLocal(tabla: string): Promise<Estado[]>{
    ...
    try {
      ...
      if (result.values) {
        ...
        throw new Error("No se encontraron datos de los estados en la base de datos")
      }
    } catch (error) {
  ...
  comparacion(endPoint: string, tabla: string): Observable<{ update: Estado[], create: Estado[] }>{
    return forkJoin({
  ...
  async update(datosDiferentes: Estado[], tabla: string) {
    ...
    try {
      for (const datos of datosDiferentes) {
        ...
        if (cambios.length > 0) {
          console.log(`Estado con id ${datos.id} actualizado con exito, ${cambios.join(', ')}`)
        } else {
          console.log(`Estado con id ${datos.id} no requiere de actualizacion`)          
        }
      }
    } catch (error) {
      console.error('Error al actualizar las Estado', error)
    }
  }

  async create (datosParaCrear: Estado[], tabla: string){
    const db = await this.sqlService.getDbName()
  ...
  async sincronizar(endPoint: string, tabla: string) {
    try {
      ...
      if (update.length > 0) {
        ...
        console.log('Estado actualizada con exito')
      } else {
      ...
      if(create.length > 0 ) {
        ...
        console.log('Datos de Estado insertados correctamente')
      }
