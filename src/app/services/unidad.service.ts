...
export class UnidadService {
  ...
  obtenerVps(endPoint: string): Observable<Unidad[]> {
    const token = localStorage.getItem('token')
    const headers = new HttpHeaders({
      Authorization: `Bearer ${token}`
    })
    return this.http.get<Unidad[]>(`${this.apiUrl}${endPoint}`, { headers })
  }

  async obtenerLocal(tabla: string): Promise<Unidad[]> {
    ...
    try {
      ...
      if (result.values) {
        ...
        throw new Error("No se encontraron datos de las Unidades en la base de datos")
      }
    } catch (error) {
  ...
  comparacion(endPoint: string, tabla: string): Observable<{ update: Unidad[], create: Unidad[] }> {
    return forkJoin({
  ...
  async update(datosDiferentes: Unidad[], tabla: string) {
    ...
    try {
      for (const datos of datosDiferentes) {
        ...
        if (cambios.length > 0) {
          console.log(`Unidad con id ${datos.id} actualizado con exito, ${cambios.join(', ')}`)
        } else {
          console.log(`Unidad con id ${datos.id} no requiere de actualizacion`)
        }
      }
    } catch (error) {
  ...
  async create(datosParaCrear: Unidad[], tabla: string) {
    const db = await this.sqlManagerService.getDbName();
  ...
  async sincronizar(endPoint: string, tabla: string) {
    try {
      ...
      if (update.length > 0) {
        ...
        console.log(`Unidad actualizada con exito`)
      } else {
      ...
      if (create.length > 0) {
        ...
        console.log('Datos de unidades insertados correctamente')
      }
